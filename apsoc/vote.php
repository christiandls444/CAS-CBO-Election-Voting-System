...
BUSINESS MANAGER</center>
	...
	<div class="col-lg-6">
	  <div class="panel panel-primary">
      <div class="panel-heading" style="background-color:#242424"><center>
FIRST REPRESENTATIVE</center>
            </div>
            <div class="panel-body" style = "background-color:;">
				<?php
					$query = $conn->query("SELECT * FROM `candidate` WHERE `position` = 'First Representative'") or die(mysqli_errno());
					while($fetch = $query->fetch_array())
					{
				?>
						<div id = "position">
     ...
     <div class="col-lg-6">
   	  <div class="panel panel-primary">
         <div class="panel-heading" style="background-color:#242424"><center>
   SECOND REPRESENTATIVE</center>
               </div>
               <div class="panel-body" style = "background-color:;">
   				<?php
   					$query = $conn->query("SELECT * FROM `candidate` WHERE `position` = 'Second Representative'") or die(mysqli_errno());
   					while($fetch = $query->fetch_array())
   					{
   				?>
   						<div id = "position">
        ...
        <div class="col-lg-6">
      	  <div class="panel panel-primary">
            <div class="panel-heading" style="background-color:#242424"><center>
      THIRD REPRESENTATIVE</center>
                  </div>
                  <div class="panel-body" style = "background-color:;">
      				<?php
      					$query = $conn->query("SELECT * FROM `candidate` WHERE `position` = 'Third Representative'") or die(mysqli_errno());
      					while($fetch = $query->fetch_array())
      					{
      				?>
      						<div id = "position">
                    ...
                    <button type="button" class="btn btn-info" style = "margin-top:10px;margin-left:5px;"><?php echo $fetch['party'].", ".$fetch['platform']?></button>
                    <button type="button" class="btn btn-info" style = "margin-top:10px;"><?php echo $fetch['firstname']." ".$fetch['lastname'].", ".$fetch['year_level']?></button>
      							<center><input type = "checkbox"  value = "<?php echo $fetch['candidate_id'] ?>" name = "sgtarm_iddd" class = "sgtarmmm"></center>
      						</div>
           ...
           <div class="col-lg-6">
         	  <div class="panel panel-primary">
               <div class="panel-heading" style="background-color:#242424"><center>
         FOURTH REPRESENTATIVE</center>
                     </div>
                     <div class="panel-body" style = "background-color:;">
         				<?php
         					$query = $conn->query("SELECT * FROM `candidate` WHERE `position` = 'Fourth Representative'") or die(mysqli_errno());
         					while($fetch = $query->fetch_array())
         					{
         				?>
         						<div id = "position">
                       ...
                       <button type="button" class="btn btn-info" style = "margin-top:10px;margin-left:5px;"><?php echo $fetch['party'].", ".$fetch['platform']?></button>
                       <button type="button" class="btn btn-info" style = "margin-top:10px;"><?php echo $fetch['firstname']." ".$fetch['lastname'].", ".$fetch['year_level']?></button>
         							<center><input type = "checkbox"  value = "<?php echo $fetch['candidate_id'] ?>" name = "sgtarm_idddd" class = "sgtarmmmm"></center>
         						</div>
...
<?php include ('script.php')?>
  ...
  <script type = "text/javascript">
      ...
      $(".sgtarmmm").on("change", function(){
        if($(".sgtarmmm:checked").length == 1)
        {
          $(".sgtarmmm").attr("disabled", "disabled");
          $(".sgtarmmm:checked").removeAttr("disabled");
        }
      else
        {
          $(".sgtarmmm").removeAttr("disabled");
        }
      });


      $(".sgtarmmmm").on("change", function(){
        if($(".sgtarmmmm:checked").length == 1)
        {
          $(".sgtarmmmm").attr("disabled", "disabled");
          $(".sgtarmmmm:checked").removeAttr("disabled");
        }
      else
        {
          $(".sgtarmmmm").removeAttr("disabled");
        }
      });
